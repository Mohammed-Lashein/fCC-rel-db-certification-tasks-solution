...
echo "$($PSQL "select distinct t.name from teams t inner join games g on t.team_id in (g.winner_id, g.opponent_id) where (year=2014 and round='Eighth-Final') order by t.name")"
...
echo  "$($PSQL "select distinct t.name from teams t full join games g on t.team_id in (g.winner_id, g.opponent_id) where t.name like 'Co%'")"
